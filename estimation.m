...
%     data_mean_emperical = data_mean_estimate_dbm;
    ...
    [alpha,epsilon,tx_height] = pathloss_estimator_hossein_method(pathloss_estimate,TX_HEIGHT,CARRIER_FREQ,packet_loss_stat,-95,TX_POWER,500,20,1);
    ALPHA = alpha(1);
    EPSILON = epsilon(1);
    TX_HEIGHT = tx_height(1);
    RX_HEIGHT = tx_height(1);
    pathloss = pathloss_gen_2ray(TX_HEIGHT,RX_HEIGHT,EPSILON,ALPHA,lambda,d_max);
    figure;plot(1:d_max,data_mean_emperical_true,'r',1:d_max,TX_POWER-pathloss,'b',1:d_max,data_mean_estimate_dbm,'g');title(['Pathloss:',' alpha :',num2str(ALPHA),' eps',num2str(EPSILON),'antenna height',num2str(TX_HEIGHT)]);legend('Field Mean RSSI', '2 Ray', 'Estimated Mean');saveas(gcf,['Plots/',file_name_string,'/','Pathloss Compare.png']);    
