function [fading_linear_cell] = nakagami_generator(params,samples_per_bin,samples_list)
...
%   Parameters : Rx2 : Range x [mu, omega]
    ...
    if samples_per_bin>0    
        ...
        for i = 1:size(params,1)
            ...
            fading_linear_cell{i} = random('nakagami',parameter(1),parameter(2),samples_list(i),1);
            fading_linear_cell{i} = fading_linear_cell{i}.*params(i,3);
        end
    end
end
